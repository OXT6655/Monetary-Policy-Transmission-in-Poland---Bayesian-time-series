...
def run_gibbs(Y_dep, X, B0, Omega0_inv, S0, nu0,
    ...
    Full conditional Sigma | B, Y:
      ...
      nu_n = nu0 + T + k
    The +k term comes from the |Sigma|^(-k/2) factor in the Matrix-Normal
    prior B|Sigma ~ MN(B0, Omega0, Sigma), which is retained in the
    conditional posterior for Sigma | B, Y (treating B as known).
    """
    T, n   = Y_dep.shape
    k      = X.shape[1]
